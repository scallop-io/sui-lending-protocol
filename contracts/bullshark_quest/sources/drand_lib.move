...
///
module bullshark_quest::drand_lib {
    ...
    /// Error codes
    const INVALID_RND_LENGTH: u64 = 0x3001;
    const INVALID_RND: u64 = 0x3002;
    ...
    /// Check whether the round is the latest round by comparing the current time with the round.
    public fun is_current_round(sig: vector<u8>, prev_sig: vector<u8>, round: u64, clock: &Clock) {
        let next_round_time = GENESIS + 30 * round;
        let epoch_time = clock::timestamp_ms(clock) / 1000;
        assert!(epoch_time < next_round_time, INVALID_RND);
    ...
    public fun safe_selection(n: u64, rnd: &vector<u8>): u64 {
        assert!(vector::length(rnd) >= 16, INVALID_RND_LENGTH);
        let m: u128 = 0;
        let i = 0;
        while (i < 16) {
            m = m << 8;
