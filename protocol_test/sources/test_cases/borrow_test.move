...
module protocol_test::borrow_test {
  ...
  public fun borrow_test() {
    ...
    let bankCoinBalance = mint_t(senario, lender, &mut bank, mintTime, usdcCoin);
    assert!(balance::value(&bankCoinBalance) == usdcAmount, 0);
    balance::destroy_for_testing(bankCoinBalance);
    ...
    deposit_collateral_t(senario, &mut position, &mut bank, ethCoin);
